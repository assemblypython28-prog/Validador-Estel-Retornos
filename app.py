...
def render_dashboard(df):
    ...
    # Otimização: usar value_counts() uma única vez
    situacao_counts = df["Situacao"].value_counts()
    total = len(df)
    com_foto = len(df[df["Foto Capturada"] == "Sim"])

    # Extrair contagens com fallback para 0
    pendentes = int(situacao_counts.get("Pendente", 0))
    aprovados = int(situacao_counts.get("Aprovado", 0))
    reparos = int(situacao_counts.get("Reparo", 0))
    avarias = int(situacao_counts.get("Avaria", 0))
    divergentes = int(situacao_counts.get("Divergente", 0))
    incompletos = int(situacao_counts.get("Incompleto", 0))
    fora_estoque = int(situacao_counts.get("Fora de estoque", 0))
    ...
    pct_incompleto = (incompletos / total * 100) if total > 0 else 0
    pct_fora_estoque = (fora_estoque / total * 100) if total > 0 else 0
    ...
    # Segunda linha: Divergente, Incompleto, Fora de estoque
    ...
    with col7:
        ...
        <div class="dashboard-card" style="border-top: 4px solid #6366F1;">
            <div class="dashboard-metric" style="color: #6366F1;">{incompletos}</div>
            <div class="dashboard-label">Incompletos</div>
            <div style="margin-top:8px;">
                <div class="progress-bar"><div class="progress-fill" style="width:{pct_incompleto}%; background:#6366F1;"></div></div>
                <small style="color:#64748B;">{pct_incompleto:.1f}%</small>
            </div>
        </div>
        """, unsafe_allow_html=True)
    with col8:
        st.markdown(f"""
        <div class="dashboard-card" style="border-top: 4px solid #EC4899;">
            <div class="dashboard-metric" style="color: #EC4899;">{fora_estoque}</div>
            <div class="dashboard-label">Fora de estoque</div>
            <div style="margin-top:8px;">
                <div class="progress-bar"><div class="progress-fill" style="width:{pct_fora_estoque}%; background:#EC4899;"></div></div>
                <small style="color:#64748B;">{pct_fora_estoque:.1f}%</small>
            </div>
        </div>
        """, unsafe_allow_html=True)
